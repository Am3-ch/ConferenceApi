...
public class AuthService
{
    ...
    public async Task<LoginResponse> Login(LoginRequest request)
    {
        ...
        // Persist refresh token (short-lived here: 1 minute) so we can rotate on use
        var refreshTokenEntity = new RefreshToken
        {
            Token = refreshToken,
            UserId = user.Id,
            ExpiresAt = DateTime.UtcNow.AddMinutes(1), // 1 minute
    ...
    public async Task<LoginResponse> Register(RegisterRequest request)
    {
        ...
        // Persist refresh token for new user (default 7 days)
        var refreshTokenEntity = new RefreshToken
        {
            Token = refreshToken,
            UserId = user.Id,
    ...
    public async Task<LoginResponse> RefreshToken(RefreshTokenRequest request)
    {
        ...
        // Save the new refresh token with a fresh expiry window
